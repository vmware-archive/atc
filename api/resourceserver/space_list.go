...
func (s *Server) ListSpaceResources(pipeline db.Pipeline) http.Handler {
	logger := s.logger.Session("list-space-resources")
	return http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		var presentedResources []atc.SpaceResource
		for _, resource := range resources {
			presentedResources = append(
				presentedResources,
				present.SpaceResource(
					resource,
					pipeline.Groups(),
					showCheckErr,
					teamName,
