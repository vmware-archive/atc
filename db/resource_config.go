...
type ResourceConfigVVV interface {
	ID() int
...
func (r *ResourceConfigDescriptor) findOrCreate(logger lager.Logger, tx Tx, lockFactory lock.LockFactory, conn Conn) (ResourceConfigVVV, error) {
	rc := &resourceConfig{
...
// XXX: Pass in lockfactory and conn
func (r *ResourceConfigDescriptor) Find(tx Tx) (ResourceConfigVVV, bool, error) {
	rc := &resourceConfig{}
	...
	if r.CreatedByResourceCache != nil {
		...
		resourceCache, found, err := r.CreatedByResourceCache.Find(tx)
		if err != nil {
			return nil, false, err
		}
