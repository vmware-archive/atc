...
func NewVersion(candidate VersionCandidate) Version {
	...
	if candidate.JobPermutationID != 0 {
		v.passed[candidate.JobPermutationID] = BuildSet{candidate.BuildID: struct{}{}}
	}
...
func (vs Versions) With(candidate VersionCandidate) Versions {
	...
	} else if candidate.JobPermutationID != 0 {
		builds, found := vs[i].passed[candidate.JobPermutationID]
		if !found {
			builds = BuildSet{}
			vs[i].passed[candidate.JobPermutationID] = builds
		}
