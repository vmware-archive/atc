...
func (p *pipeline) LoadVersionsDB() (*algorithm.VersionsDB, error) {
	...
	rows, err = psql.Select("vr.id, vr.check_order, rs.id, i.build_id, i.name, b.job_combination_id").
	...
	for rows.Next() {
		...
		err = rows.Scan(&input.VersionID, &input.CheckOrder, &input.ResourceSpaceID, &input.BuildID, &input.InputName, &input.JobCombinationID)
		if err != nil {
			return nil, err
		}
