...
var buildsQuery = psql.Select("b.id, b.name, c.id, j.id, t.id, b.status, b.manually_triggered, b.scheduled, b.engine, b.engine_metadata, b.public_plan, b.start_time, b.end_time, b.reap_time, j.name, b.pipeline_id, p.name, t.name, b.nonce, b.tracked_by").
	From("builds b").
...
func scanBuild(b *build, row scannable, encryptionStrategy encryption.Strategy) error {
	...
	err := row.Scan(&b.id, &b.name, &jobCombinationID, &jobID, &b.teamID, &status, &b.isManuallyTriggered, &b.scheduled, &engine, &engineMetadata, &publicPlan, &startTime, &endTime, &reapTime, &jobName, &pipelineID, &pipelineName, &b.teamName, &nonce, &trackedBy)
	if err != nil {
		return err
	}
